...
namespace Moda.KString
{
    public static class KoreanStringExtension
    {
        ...
        public static string KAppendJosa(this string s, JosaType josaType)
        {
            ...
            {
                case JosaType.EN:
                    return KAppendJosa(s, "은", "는");
                case JosaType.ER:
                    return KAppendJosa(s, "을", "를");
                case JosaType.WG:
                    return KAppendJosa(s, "와", "과");
                case JosaType.YDD:
                    return KAppendJosa(s, "이다", "다");
                case JosaType.YG:
                    return KAppendJosa(s, "이", "가");
                case JosaType.ERR:
                    return KAppendJosa(s, "으로", "로");
        ...
        public static class PredefinedStringMatcher
        {
            ...
            {
                if (KIndexOf(s, keyword) == -1)
                {
                    return null;
                }
                else
                {
                    return s.Substring(KIndexOf(s, keyword), keyword.Length);
                }
            }
        }
        ...
        public static class PredefinedIndexOfFinder
        {
            ...
            private static int KoreanJamoIndexOfFinder(string s, string keyword)
            {
                ...
                for (int i = 0; i <= max; i++)
                {
                    ...
                    if (KEquals(s[i], firstKeyword) == false)
                    {
                        while (++i <= max && KEquals(s[i], firstKeyword) == false) ; // do nothing but traversing
                    }
                    ...
                    if (i <= max)
                    {
                        ...
                        {
                            if (KEquals(s[sIndex], keyword[keywordIndex]))
                            {
                                continue;
                            }
                            else
